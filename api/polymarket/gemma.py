...
from utils.http import ThreadLocalSessionMixin
...
class GammaClient(ThreadLocalSessionMixin):
    ...
    def get_events(
        ...
        limit: int = 100,
        offset: int = 0,
        order: Optional[str] = None,           # Field to order by (e.g., "id", "startTime")
        ascending: Optional[bool] = None,       # Sort direction: True for ascending, False for descending
        closed: Optional[bool] = None,          # Filter by closed status
        active: Optional[bool] = None,          # Filter by active status
        tag_id: Optional[int] = None,           # Filter by tag ID
        series_id: Optional[int] = None,        # Filter by series ID (for sports)
        slug: Optional[str] = None,             # Filter by event slug
        exclude_tag_id: Optional[int] = None,   # Exclude specific tag ID
        related_tags: Optional[bool] = None,    # Include related tag markets
    ) -> Optional[Dict[str, Any]]:
        """
        Fetch events from the Gamma API.

        Args:
            limit: Number of events to fetch (max per API)
            offset: Pagination offset
            order: Field to order by (e.g., "id", "startTime")
            ascending: Sort direction (True=ascending, False=descending)
            closed: Filter by closed status (True/False)
            active: Filter by active status (True/False)
            tag_id: Filter by tag ID
            series_id: Filter by series ID (for sports leagues)
            slug: Filter by event slug
            exclude_tag_id: Exclude events with this tag ID
            related_tags: Include related tag markets

        Returns:
            Dictionary with events or None if request failed
        """
        ...
        if ascending is not None:
            params["ascending"] = str(ascending).lower()  # "true" or "false"
